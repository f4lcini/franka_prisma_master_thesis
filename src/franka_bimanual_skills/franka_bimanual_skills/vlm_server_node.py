...
class VlmServerNode(Node):
    ...
    async def execute_callback(self, goal_handle):
        ...
        system_prompt = (
            ...
            "If an object must be moved between zones reachable by different arms (e.g. from far left to far right), use a 'shared' handover:\n"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    self.get_logger().info(f"Waiting {base_delay} seconds before retrying...")
