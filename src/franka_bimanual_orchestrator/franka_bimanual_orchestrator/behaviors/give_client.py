...
class GiveActionClient(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        ...
        if target_pose_dynamic:
            self.logger.info(f"[{self.name}] Sending GIVE Goal with DYNAMIC Pose for {target_arm}")
            goal_msg.handover_pose = target_pose_dynamic
        else:
            self.logger.info(f"[{self.name}] Sending GIVE Goal for {target_arm} using LABEL: {target_loc}")
