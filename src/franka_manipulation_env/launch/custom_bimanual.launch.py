...
def robot_gripper_spawner(
    ...
    return [
        Node(
            ...
            # If using Gazebo or Fake Hardware, we shouldn't attempt a real libfranka connection to the gripper
            condition=IfCondition(
                PythonExpression([
                    "'", LaunchConfiguration('use_fake_hardware', default='false'), "' == 'false' and ",
                    "'", LaunchConfiguration('use_gazebo', default='false'), "' == 'false'"
                ])
            ),
        )
    ]
...
def generate_launch_description():
    ...
    launch_description = LaunchDescription([
        ...
        Node(
            ...
            parameters=[{
                'source_list': ["/joint_states", "franka1/franka_gripper/joint_states", "franka2/franka_gripper/joint_states"],
                'rate': 1000.0,
        ...
        IncludeLaunchDescription(
            ...
            launch_arguments={'gz_args': f"{os.path.join(pkg_franka_env, 'worlds', 'bimanual_custom.world')} -r --render-engine ogre"}.items(),
            condition=IfCondition(use_gazebo)
        ),
