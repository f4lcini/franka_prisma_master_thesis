# Bimanual Franka FR3 Framework: Technical Deep-Dive
...
- **Alignment**: Hardware-accelerated depth-to-RGB alignment is enabled via the `align_depth.enable:=true` parameter, ensuring that every RGB pixel $(u, v)$ has a corresponding depth value $Z$ at the same coordinate.
- **QoS Requirements**: The driver publishes color and depth streams using the **Best Effort** Reliability Policy. Subscribers must match this policy to avoid dropped frames in high-bandwidth scenarios.

### 1.2 Object Detection (YOLOv8)
The system utilizes the **YOLOv8n (Nano)** model for real-time inference on the RGB stream (640x480).
- **Inference**: Orchestrated via a custom ROS 2 Action Server (`/detect_object`).
...
### 1.3 3D Localization (Inverse Pinhole Deprojection)
...
### 1.4 Orientation Estimation (3D PCA)
The system estimates the object's 3D orientation to ensure precise, stable grasping:
1. **Local Point Cloud**: A subset of depth pixels within the bounding box is deprojected into a local 3D point cloud $P = \{p_1, p_2, ..., p_n\}$.
2. **Covariance Analysis**: The 3x3 covariance matrix $C = \frac{1}{n} (P - \bar{P})^T (P - \bar{P})$ is computed.
3. **Eigen-Decomposition**: Using `np.linalg.eigh` (via `numpy.linalg`), the system extracts eigenvalues and eigenvectors.
4. **Principal Axis**: The 1st eigenvector represents the principal axis of the object.
5. **Quaternion Extraction**: A rotation matrix is constructed from the eigenvectors and converted into a $[x, y, z, w]$ quaternion using **SciPy's `Rotation`** class. To ensure a "top-down" preference, the system projects the primary axis onto the XY plane before normalization.

---

## 2. Reasoning Layer: Cognitive Neuro-Symbolic Planning
...
The framework leverages **Google Gemini 2.5 Flash** for its multimodal capabilities and state-of-the-art low latency.
- **Prompt Engineering**: The **System Prompt** embeds hard-coded kinematic constraints and workspace topology.
- **Chain-of-Thought (CoT)**: The model is forced to output its internal logic in a `reasoning` field before finalizing categorical decisions, significantly increasing planning accuracy and reliability.
...
- **Singularity Avoidance**: Reaching across the workspace midline is strictly prohibited.
- **Handover Estimation**: For bimanual transfers, Gemini estimates a safe $Z$ rendezvous height centered between the arms or utilizes the `shared` table relay zone.
...
- **Schema Enforcement**: The `TaskPlan` and its constituent skills (e.g., `PickSkill`, `PlaceSkill`, `WaitSkill`, `MoveHomeSkill`, `FindObjectSkill`) are defined as Pydantic `BaseModel` classes. 
...
class RobotSkill(BaseModel):
    action: Literal["PICK", "PLACE", "WAIT", "MOVE_HOME", "FIND_OBJECT"]
    arm: Literal["left_arm", "right_arm"]
    target_name: Optional[str] = Field(None, description="Object name for YOLO")
    target_location: Optional[str] = Field(None, description="Location like 'shared' or 'box'")

class TaskPlan(BaseModel):
    reasoning: str = Field(..., description="Explain the bimanual strategy")
    sequence: List[RobotSkill]
```
...
## 3. Behavior Tree Orchestration: The Nervous System
The high-level logic and mission flow are managed by a **Behavior Tree (BT)** using `py_trees`.

### 3.1 Architecture Overview: Parallel Lane Execution
...
### 3.2 Non-Blocking Asynchronous Behaviors
Each arm skill (e.g., `PickActionClient`) is implemented using asynchronous ROS 2 Action Clients (`send_goal_async`). When a skill is triggered, it returns `RUNNING` to the BT, allowing the tree to continue ticking ($5\text{Hz}-10\text{Hz}$) and processing the other arm's state in the same cycle.
...
### 3.4 Advanced Recovery and Fault Tolerance
- **ReplanTrigger**: If a `PICK` fails due to "Object Missed," the tree triggers a new VLM cycle instead of aborting.
- **MissionAbort**: Critical failures (IK timeout, move rejected) trigger a safety sequence that moves both arms to the `MOVE_HOME` position and stops the mission.
- **UnmatchedActionLogger**: A diagnostic node that logs errors if the dispatcher fails to match a VLM action to a registered skill.

---

## 4. Control Architecture: MoveGroup & Custom Hardware Interface
The framework overcomes the limitations of standard drivers through a custom architecture integrating `ros2_control`, `MoveIt 2`, and `libfranka`.
...
Hardware integration is managed by the custom `franka_ros2_multimanual` package, extending `libfranka` capabilities:
...
### 4.3 State Hub: Specialized Readers
...
Achieving robust bimanual operation in a Python environment requires solving critical technical challenges.
...
- When a Python thread waits for a response from a ROS 2 Action Server (MoveIt) or a TF transform, it **releases the Global Interpreter Lock (GIL)**.
...
### 5.4 Deterministic Synchronization: The Rendezvous Handshake
Direct coordination between arms for mid-air handovers is achieved via a **Dual-Service Rendezvous** pattern:
1. **Donor Arm** calls `/donor_ready`.
2. **Recipient Arm** calls `/recipient_ready`.
3. **HandoverCoordinator**: Blocks both calls using `threading.Event()`. Once both are registered, it releases them simultaneously.
4. **Non-Polling Wait**: Unlike "busy-wait" loops, the `.wait()` method blocks the thread in a low-power state and is woken up by the OS kernel as soon as the other thread calls `.set()`. 
5. **Design Note on Wrench Feedback**: While the framework architecture is designed to support closed-loop **Wrench Feedback** ($O\_F\_ext\_hat\_K$ force spike detection) for reflex-based release, the current implementation utilizes deterministic logic-gate synchronization to ensure safety in common industrial handover scenarios.
...
1. **Inference**: The `VlmActionClient` sends scene metadata and image to Gemini 2.5 Flash.
...
2. **Dispatching**: The `Dispatcher` selector triggers the corresponding Action Client based on the current action name.
...
2. **Fail-Stop**: If one arm encounters an IK failure or timeout, the specific lane halts. The partner arm will timeout its wait events, preventing mid-air collisions.

---

## 7. Package Architecture: Modular System Design
The framework orchestrates 6 custom ROS 2 packages into a cohesive robotic system:

| Package Name | Functional Role | Key Components |
| :--- | :--- | :--- |
| `franka_bimanual_orchestrator` | Frontend: Behavior Tree and Gemini VLM mission logic. | `main_engine.py`, `py_trees` nodes. |
| `franka_bimanual_skills` | Backend: ROS 2 Skill Servers and repertoire. | `Pick/Place/Handover` servers, `robot_control_api.py`. |
| `franka_bimanual_bringup` | Entry point for mission-level testing and hardware bringup. | Launch files, environment configs. |
| `franka_custom_interfaces` | Communication: Custom `.action` and `.srv` definitions. | `VlmQuery`, `HandoverReady`, `ParallelMove`. |
| `franka_bimanual_planner` | C++ Motion Planning: Unified MoveIt/Pilz interface. | `ParallelMove` server. |
| `franka_ros2_multimanual` | Driver: Low-level C++ hardware integration. | `franka_mm_hardware_interface`, `libfranka` wrappers. |
...
## 9. Workspace Refactoring: Naming Strategy
...
### 10.2 The Backend (The "Engine Room")
...
## 11. Deep-Dive Design Strengths

### 11.1 Concurrency and the GIL
...
### 11.2 Industrial Safety via Quintic Polynomials
...
## 12. Future Work: Elevating the State-of-the-Art
...
1. **Dynamic Kinematic Load Balancing (Yoshikawa Index)**: Calculating the **Yoshikawa Index** ($\sqrt{det(J \cdot J^T)}$) for real-time optimal task assignment.
2. **Closed-Loop Semantic Recovery**: VLM-based visual re-interrogation upon failure ("Re-Pick with +20mm depth offset") without human intervention.
3. **Migration to Edge-VLM (Local AI)**: Running quantized models (e.g., **LLaVA**, **Qwen-VL**) running on local GPUs/NPUs to eliminate cloud dependency.
4. **Native Cartesian Impedance Control**: Transitioning to compliance during the exact moment of object contact via a `cartesian_impedance_controller`.

---

## 13. Experimental Validation and Metrics
...
### 13.1 Phase 1: Simulation (Gazebo / Ignition)
...
#### 13.1.1 Zero-Shot Cognitive Scalability (VLM vs. Scene Complexity)
...
#### 13.1.2 Multithreading Validation (The GIL Test)
...
#### 13.1.3 Determinism and Fail-Stop Safety
...
#### 13.2.2 Handover Synchronization (The Rendezvous Handshake)
...
#### 13.2.3 Dynamic Obstacle Reactivity
...
## 14. Laboratory Deployment Readiness Checklist
...
### 14.1 Host Machine Requirements (Physical PC)
...
### 14.2 Software & Middleware Configuration
...
### 14.3 Deployment Hardware Verification
...
*Document Version 2.0 - Authored by Falco Robotics.*
