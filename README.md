...
This project is containerized to ensure a reproducible environment and clean dependency management. The Docker setup has been recently updated to include `ros-humble-apriltag-ros` for camera calibration and `ultralytics` for YOLO-based perception.
...
To run the ROS 2 pipelines, open a terminal inside the running container:
```bash
